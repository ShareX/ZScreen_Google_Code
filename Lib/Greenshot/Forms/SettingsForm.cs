...
namespace Greenshot
{
    ...
    public partial class SettingsForm : Form
    {
        ...
        private void SetEmailFormat()
        {
            ...
            this.combobox_emailformat.Items.Clear();
            if (EmailConfigHelper.HasMAPI())
            {
                ...
                {
                    this.combobox_emailformat.Items.AddRange(new object[] { EmailFormat.MAPI, EmailFormat.OUTLOOK_TXT, EmailFormat.OUTLOOK_HTML });
                }
                else
                {
                    // Force MAPI in configuration if no Outlook
                    coreConfiguration.OutputEMailFormat = EmailFormat.MAPI;
                    this.combobox_emailformat.Items.AddRange(new object[] { EmailFormat.MAPI });
                }
            }
            else
            {
        ...
        private void SaveSettings()
        {
            ...
            // Fix for Bug #3405376
            if (combobox_emailformat.SelectedItem != null)
            {
