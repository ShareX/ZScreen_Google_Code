    ...
    public partial class ZScreen : Form
    {
        ...
        private void SetupScreen()
        {
            ...
            if (Program.conf.DekiWikiAccountList == null || Program.conf.DekiWikiAccountList.Count == 0)
            {
                ...
                DekiWikiLoad(new DekiWikiAccount());
            }
            else
            {
                DekiWikiSetup(Program.conf.DekiWikiAccountList);
                if (lbDekiWikiAccounts.Items.Count > 0)
                {
                    lbDekiWikiAccounts.SelectedIndex = Program.conf.DekiWikiSelected;
                }
            }

            cbDekiWikiForcePath.Checked = Program.conf.DekiWikiForcePath;
        ...
        private string GetFilePath(MainAppTask.Jobs job)
        {
            ...
            // HACK: the manual naming window can change the destination
            // so redo the menus now
            switch (Program.conf.ScreenshotDestMode)
            {
                ...
                case ImageDestType.DEKIWIKI:
                    CheckSendToMenu(tsmDestDekiWiki);
                    break;
        ...
        private void WriteImage(MainAppTask t)
        {
            if (t.MyImage != null)
            {
                ...
                // Update destination again since the input box can now change it
                t.ImageDestCategory = Program.conf.ScreenshotDestMode;
                t.TextDestCategory = Program.conf.TextDestMode;
            }
        }
        ...
        private void BwApp_RunWorkerCompleted(object sender, RunWorkerCompletedEventArgs e)
        {
            ...
            catch (Exception ex)
            {
                ...
                // HACK - for canceling at the naming box
                niTray.Icon = Resources.zss_tray;
            }
            finally
            {
        ...
        */
        ...
        private void OpenLocalFile()
        {
            if (lbHistory.SelectedItem != null)
            {
                ...
                if (!string.IsNullOrEmpty(hi.LocalPath))
                {
                    ...
                    {
                        MessageBox.Show("Could not open the screen shot.");
                    }
                }
            }
        }
        ...
        private void pbHistoryThumb_Click(object sender, EventArgs e)
        {
            ...
            if (hi != null && File.Exists(hi.LocalPath))
            {
                ...
                else
                {
                    ...
                    {
                        MessageBox.Show("Could not open the screenshot in an external viewer.");
                    }
                }
            }
        }
        ...
        private string DekiWikiAdd(DekiWikiAccount acc)
        {
            return acc.Name;
        }

        private void DekiWikiLoad(DekiWikiAccount acc)
        {
            txtDekiWikiName.Text = acc.Name;
            txtDekiWikiUrl.Text = acc.Url;
            txtDekiWikiUsername.Text = acc.Username;
            txtDekiWikiPassword.Text = acc.Password;
            gbDekiWikiAccount.Text = string.Format("Settings: {0}", acc.Name);
        }
        ...
        private void DekiWikiSetup(IEnumerable<DekiWikiAccount> accs)
        {
            if (accs != null)
            {
                lbDekiWikiAccounts.Items.Clear();
                ...
                {
                    lbDekiWikiAccounts.Items.Add(DekiWikiAdd(acc));
                }
            }
        }

        private DekiWikiAccount GetDekiWikiAccountFromFields()
        {
            TrimFTPControls();

            DekiWikiAccount acc = new DekiWikiAccount
            {
                Name = txtDekiWikiName.Text,
                Url = txtDekiWikiUrl.Text,
                Username = txtDekiWikiUsername.Text,
                Password = txtDekiWikiPassword.Text
            };

            return acc;
        }

        private bool ValidateDekiWiki()
        {
            Control[] controls = { txtDekiWikiUrl, txtDekiWikiUsername, txtDekiWikiPassword };
        ...
        private void UpdateDekiWiki()
        {
            if (ValidateDekiWiki()
                && lbDekiWikiAccounts.SelectedIndices.Count == 1 && lbDekiWikiAccounts.SelectedIndex != -1)
            {
                lblDekiWikiStatus.Text = "Updated.";

                DekiWikiAccount acc = GetDekiWikiAccountFromFields();

                if (Program.conf.DekiWikiAccountList != null)
                {
                    Program.conf.DekiWikiAccountList[lbDekiWikiAccounts.SelectedIndex] = acc; //use selected index instead of 0
                }
                else
                {
                    Program.conf.DekiWikiAccountList = new List<DekiWikiAccount> { acc };
                }

                lbDekiWikiAccounts.Items[lbDekiWikiAccounts.SelectedIndex] = DekiWikiAdd(acc);
            ...
            {
                lblDekiWikiStatus.Text = "Not Updated.";
            }
        }

        // TODO
        private void lbDekiWikiAccounts_SelectedIndexChanged(object sender, EventArgs e)
        {
            int sel = lbDekiWikiAccounts.SelectedIndex;
            ...
            {
                DekiWikiLoad(Program.conf.DekiWikiAccountList[sel]);
                Program.conf.DekiWikiSelected = lbDekiWikiAccounts.SelectedIndex;
                //RewriteFTPRightClickMenu();
            }
        }

        private void btnDekiWikiUpdate_Click(object sender, EventArgs e)
        {
            UpdateDekiWiki();
        }

        private void btnDekiWikiClear_Click(object sender, EventArgs e)
        {
            DekiWikiLoad(new DekiWikiAccount());
        }

        private void btnDekiWikiAdd_Click(object sender, EventArgs e)
        {
            if (ValidateDekiWiki())
            {
                DekiWikiAccount acc = GetDekiWikiAccountFromFields();
                ...
                lbDekiWikiAccounts.Items.Add(DekiWikiAdd(acc));
                lbDekiWikiAccounts.SelectedIndex = lbDekiWikiAccounts.Items.Count - 1;
            }
            else
            {
                lblDekiWikiStatus.Text = "Not Updated.";
        ...
        private void btnDekiWikiRemove_Click(object sender, EventArgs e)
        {
            int sel = lbDekiWikiAccounts.SelectedIndex;
            ...
            if (sel != -1)
            {
                ...
                lbDekiWikiAccounts.Items.RemoveAt(sel);

                if (lbDekiWikiAccounts.Items.Count > 0)
                {
                    lbDekiWikiAccounts.SelectedIndex = (sel > 0) ? (sel - 1) : 0;
                }
            }
        }

        private void btnDekiWikiTest_Click(object sender, EventArgs e)
        {
            // Set the status
            lblDekiWikiStatus.Text = "Testing...";
            lblDekiWikiStatus.Update();
            ...
            try
            {
                ...
                lblDekiWikiStatus.Text = "Success!"; //Success
            }
            catch (Exception t)
            {
                lblDekiWikiStatus.Text = t.Message;
            }
        }

        #endregion

        #region DekiWiki Menu Items

        private void tsmDestDekiWiki_Click(object sender, EventArgs e)
        {
            cboScreenshotDest.SelectedIndex = (int)ImageDestType.DEKIWIKI;
        }

        private void tsmDekiWikiPath_Click(object sender, EventArgs e)
        {
            int count = Program.conf.DekiWikiAccountList.Count;
            int index = Program.conf.DekiWikiSelected;

            if (count == 0 || index < 0 || index >= count) {
                MessageBox.Show("Please set up and select a Mindtouch account first.");
                return;
            }

            DekiWikiAccount acc = Program.conf.DekiWikiAccountList[index];
            DekiWikiPath pathDialog = new DekiWikiPath(ref acc);

            if (pathDialog.ShowDialog() == DialogResult.OK)
            {
                DekiWiki.savePath = pathDialog.path;
            }
        }

        #endregion

        private void cbDekiWikiForcePath_CheckedChanged(object sender, EventArgs e)
        {
            Program.conf.DekiWikiForcePath = cbDekiWikiForcePath.Checked;
        }
    }
}