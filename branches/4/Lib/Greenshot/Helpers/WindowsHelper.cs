...
namespace Greenshot.Helpers  {
	...
	public class WindowDetails : IEquatable<WindowDetails>{
		...
		public Image PrintWithVScroll() {
			...
			float pixelsPerItem = (float)clientRectangle.Height / (float)scrollinfo.nPage;
			int height = (int)(pixelsPerItem * scrollinfo.nMax);
			int pageSkip = scrollinfo.nPage -1;
			LOG.DebugFormat("pixelsPerItem {0}", pixelsPerItem);
			if (width * height == 0) {
				...
				do {
					...
					using (Image printedWindow = PrintWindow()) {
						if (printedWindow != null) {
							graphicsTarget.DrawImage(printedWindow, new Point(0, (int)(pixelsPerItem * location)));
		...
		public static List<WindowDetails> GetVisibleWindows() {
			...
			foreach(WindowDetails window in allWindows) {
				...
				Rectangle windowRect = window.ClientRectangle;
				if (windowRect.Width * windowRect.Height <= 2) {
					continue;
				}
