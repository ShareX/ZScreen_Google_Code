...
namespace ZSS.Colors
{
    public class ColorUserControl : UserControl
    {
        ...
        protected void SetSliderMarker()
        {
            ...
            {
                case DrawStyle.Hue:
                    lastPos.Y = (height - 1) - Round(height * SetColor.HSB.Hue);
                    break;
                case DrawStyle.Saturation:
                    lastPos.Y = (height - 1) - Round(height * SetColor.HSB.Saturation);
                    break;
                case DrawStyle.Brightness:
                    lastPos.Y = (height - 1) - Round(height * SetColor.HSB.Brightness);
                    break;
                case DrawStyle.Red:
                    lastPos.Y = (height - 1) - Round(height * (double)SetColor.RGB.Red / 255);
                    break;
                case DrawStyle.Green:
                    lastPos.Y = (height - 1) - Round(height * (double)SetColor.RGB.Green / 255);
                    break;
                case DrawStyle.Blue:
                    lastPos.Y = (height - 1) - Round(height * (double)SetColor.RGB.Blue / 255);
                    break;
            }
            lastPos = GetPoint(lastPos);
        }
